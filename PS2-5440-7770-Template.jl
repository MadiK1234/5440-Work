...
include(joinpath(_PATH_TO_NOTEBOOK,"src\\Stoichiometric.jl"))
...
html"""
<p style="font-size:20px;">Madison Keiser (mmk274) </br>
...
begin
    ...
    push!(reaction_array,"v₂,2-(Nomega-L-arginino)succinate,L-arginine+fumarate,false")
    push!(reaction_array,"v₃,L-arginine+H2O,Urea+L-ornithine,false")
    push!(reaction_array,"v₄,L-ornithine+carbamoylphosphate,L-citrulline+phosphate,false")
    push!(reaction_array,"v₅,2*L-arginine+3*NADPH+3*H+4*O2,2*L-citrulline+2*nitricoxide+3*NADP+4*H2O,true")
    ...
    push!(reaction_array,"b₆,phosphate,∅, true")
    push!(reaction_array,"b₇,diphosphate,∅, true")
    ...
    # compute the stoichiometric matrix -AND binary stoichiometric matrix
    (S, species_array, reaction_name_array) = build_stoichiometric_matrix(reaction_array);
    B=binary_stoichiometric_matrix(S);
...
begin
	...
	PM=lib.expa(S)
	#P constrains the extreme pathways (row) and N is the balanced array (should be all zeros)-
	P=PM[:,1:ℛ]
	S=PM[:,(ℛ+1):end]
end
